...
void shortestpath(int F,int edgeCost[][MaxVertexSize],int D[],int n,short int selected[]);
...
void shortestpath(int F,int edgeCost[][MaxVertexSize],int D[],int n,short int selected[])
{
  ...
  {
    t=chooseMin(D,n,selected);      /* 找出D中尚未被選擇過最小的 */
    selected[t]=TRUE;
    for(I=1;I<=n;I++)
    { 
      if(selected[I]!=TRUE)              /* 已被選過可直接略過 */
